...
func post_guest_score(
	...
	if success and _http_post_request_busy:
		printerr(
			"[Quiver Leaderboards] Couldn't post score because POST request is in progress"
		)
		success = false
	...
	if success:
		...
		if error != OK:
			printerr(
				"[Quiver Leaderboards] There was an error posting a score."
			)
			success = false
		else:
			...
			# regardless of the automatically_retry option.
				...
				printerr(
					"[Quiver Leaderboards] There was an irrecoverable error posting a score."
				)
				retry = false
				success = false
	...
	if not success and retry:
		_handle_failed_post(
			"guest", leaderboard_id, float(score), nickname, metadata, timestamp
		)
...
func get_scores(
	...
	var query_string := (
		"?offset=%d&limit=%d&start_time=%f&end_time=%f"
		% [offset, limit, start_time, end_time]
	)
	var path_to_use := GET_SCORES_PATH
	var token = Secret.API_KEY
	...
	return await _get_scores_base(
		leaderboard_id, token, path_to_use, query_string
	)
...
func get_player_scores(
	...
	return await _get_scores_base(
		leaderboard_id,
		PlayerAccounts.player_token,
		GET_PLAYER_SCORES_PATH,
		query_string
	)
...
func _get_scores_base(
	leaderboard_id: String, token: String, path: String, query_string := ""
):
	var scores := []
	var has_more_scores := false
	if not token:
		printerr(
			"[Quiver Leaderboards] Can't fetch scores due to missing token"
		)
		return {
			"scores": [], "has_more_scores": false, "error": "Missing token"
		}
	...
	if error != OK:
		...
		if response_code >= 200 and response_code <= 299:
			...
			if parsed_data is Dictionary:
				...
				printerr(
					"[Quiver Leaderboards] There was an error while parsing score data"
				)
				error_msg = "Error parsing response"
		else:
			printerr(
				"[Quiver Leaderboards] There was an error fetching scores."
			)
			error_msg = "Request failed, HTTP code %d" % response_code
	_http_get_request_busy = false
	return {
		"scores": scores, "has_more_scores": has_more_scores, "error": error_msg
	}
...
func _handle_failed_post(
	...
	if _failed_queue.size() > MAX_FAILED_QUEUE_SIZE:
		...
		if _failed_queue_file:
			_failed_queue_file.close()
		_failed_queue_file = FileAccess.open(
			FAILED_QUEUE_FILE_NAME, FileAccess.WRITE
		)
		if _failed_queue_file:
			...
			printerr(
				"[Quiver Leaderboards] Failed to save failed request to disk."
			)
	else:
		if not _failed_queue_file:
			if FileAccess.file_exists(FAILED_QUEUE_FILE_NAME):
				_failed_queue_file = FileAccess.open(
					FAILED_QUEUE_FILE_NAME, FileAccess.READ_WRITE
				)
